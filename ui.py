...
class ExcelSorterApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        help_menu.add_command(label="About", command=lambda: messagebox.showinfo(
            "About", "Excel Sheet Sorter\nVersion 1.0\nDeveloped by NKau$hal!!!!")
            )
    ...
    def setup_ui(self):
        ...
        upload_frame.pack(side="right", fill="y", padx=(8, 8), pady=6)
        ...
        tk.Label(content_frame, text="Select Excel File:", bg="#dfe6ee",
                 font=("Helvetica", 12, "bold")).grid(row=0, column=0, sticky="w", pady=(0, 0))
        ...
        batch_check.pack(side="left", padx=(0, 2))
        ...
        tk.Label(content_frame,
                 ...
                 bg="#dfe6ee", font=("Helvetica", 8, "bold"), fg="#555").grid(row=2, column=0, sticky="w", pady=(0, 0))
        ...
        self.sheet_listbox = tk.Listbox(listbox_container, selectmode="single",
            font=("Arial", 11), bg="white", activestyle="dotbox", height=10, width=67)
        ...
        sort_menu = ttk.OptionMenu(adv_frame, self.sort_mode_var,
            "alpha",
            "alpha",
            "reverse_alpha",
            "numeric_suffix",
        ...
        self.log_text = tk.Text(self.log_frame, height=8, state="normal", wrap="word")
    ...
    def _load_paths(self, paths: list):
        ...
        if len(paths) == 1:
            ...
            if loaded:
                ...
                # Auto-detect month-based sheets and preselect calendar sort mode.
                try:
                    from sheet_rules import contains_month_sheets
                    if contains_month_sheets(sheets):
                        # Auto-switch sort mode only if user has not manually selected another
                        current_mode = self.sort_mode_var.get()
                        if current_mode in ("alpha", "reverse_alpha", "numeric_suffix"):
                            self.sort_mode_var.set("Jan→Dec")
                            self._log("[INFO] Month-based sheets detected → auto-selected 'Jan→Dec'.")
                except (ImportError, AttributeError) as exc:
                    # Log specific issues without interrupting UI flow
                    self._log(f"[WARN] Month detection unavailable: {exc}")
    ...
    def sort_sheets(self):
        ...
        if mode:
            ...
            if mode == "alpha":
                ...
                # Wrap alpha_key to invert sort order.
                def _rev(ws):
                    """Reverse alphabetical sort key for sheet objects."""
                    return tuple(-ord(char) for char in ws.title.lower()[:16])
                key_func = _rev
            elif mode == "numeric_suffix":
                key_func = numeric_suffix_key
            elif mode == "Jan→Dec":
                key_func = month_order_key
            elif mode == "Dec→Jan":
                key_func = month_order_desc_key
